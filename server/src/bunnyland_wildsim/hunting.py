...
class HuntHandler:
    ...
    def execute(self, ctx: HandlerContext, command: SubmittedCommand) -> HandlerResult:
        ...
        if hunt_score(ctx.world, character, scent) < SUCCESS_THRESHOLD:
            ...
            operations = []
            if damage > 0.0 and character.has_component(HealthComponent):
                health_component = character.get_component(HealthComponent)
                updated_health = replace(
                    health_component,
                    current=health_component.current - damage,
                )
                operations.append(SetComponent(character.id, updated_health))
        ...
        operations = [
            ...
            DeleteEntity(target_id),
        ]
        if predator and character.has_component(HealthComponent):
            health = character.get_component(HealthComponent)
            operations.append(
                SetComponent(
                    character.id,
                    replace(health, current=health.current - PREDATOR_KILL_DAMAGE),
                )
            )
        return planned(
            MutationPlan(tuple(operations)),
